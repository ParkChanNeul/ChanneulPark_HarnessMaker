...
2명 이상의 에이전트가 협업할 때 **가장 먼저 검토하는 기본 모드**. parent가 custom subagent들을 명시적으로 dispatch하고, task plan과 dependency tracking으로 조율한다. Codex에서는 subagent끼리 직접 통신하지 않으므로 parent가 반환 결과와 `_workspace/` 산출물을 비교한 뒤 follow-up 입력을 다시 보낸다.
...
1. 사용할 custom agent 정의 확인:
   ```
   .codex/agents/{teammate-1}.toml
   .codex/agents/{teammate-2}.toml
   ```

2. parent dispatch plan 작성:
   ```yaml
   subagents:
     - name: {teammate-1}
       agent_file: .codex/agents/{teammate-1}.toml
       input: _workspace/00_input/request.md
       expected_output: _workspace/{phase}_{teammate-1}_{artifact}.md
       completion_criteria:
         - "{검증 가능한 완료 조건}"
     - name: {teammate-2}
       agent_file: .codex/agents/{teammate-2}.toml
       input: _workspace/00_input/request.md
       expected_output: _workspace/{phase}_{teammate-2}_{artifact}.md
       completion_criteria:
         - "{검증 가능한 완료 조건}"
   ```
...
**실행 방식:** parent-mediated coordination

parent는 독립 작업만 병렬로 dispatch한다. 각 subagent는 결과를 parent에게 반환하고, 파일 산출물이 필요한 경우 지정된 `_workspace/` 경로에 초안을 남긴다. parent는 결과를 모두 기다린 뒤 충돌·누락·불확실 evidence를 비교한다.

**팀원 간 통신 규칙:**
- {teammate-1}의 발견 중 {teammate-2} 검토가 필요한 내용은 parent가 follow-up 입력으로 전달
- {teammate-2}는 작업 완료 시 결과를 파일로 저장하고 리더에게 알림
- 팀원이 다른 팀원의 결과가 필요하면 parent에게 필요한 evidence와 질문을 반환
- parent는 상충 evidence를 삭제하지 않고 출처와 함께 병기
...
- 특정 팀원이 막혔을 때 parent가 follow-up로 지시 또는 작업 재할당
- 전체 진행률은 parent task plan으로 확인

### Phase 4: {후속 작업 — 예: 검증/통합}
1. 모든 팀원의 작업 완료 대기 (parent task plan으로 상태 확인)
2. 각 팀원의 산출물을 Read로 수집
3. {통합/검증 로직}
4. parent가 최종 산출물 생성: `{output-path}/{filename}`

### Phase 5: 정리
1. 진행 중인 subagent thread가 없음을 확인
2. follow-up 대상이 없으면 subagent 작업 종료로 간주
3. `_workspace/` 디렉토리 보존 (중간 산출물은 삭제하지 않음 — 사후 검증·감사 추적용)
4. 사용자에게 결과 요약 보고

> **팀 재구성이 필요한 경우:** Phase별로 다른 전문가 조합이 필요하면, 현재 Phase의 subagent 결과를 `_workspace/`에 보존한 뒤 다음 Phase의 subagent 묶음을 새로 dispatch한다. 이전 Phase의 산출물은 `_workspace/`에 보존되므로 새 subagent 입력에 명시한다.

## 데이터 흐름

```
[parent] → dispatch {teammate-1} → artifact-1.md
   │              ↓
   │        returned evidence
   ├────→ dispatch {teammate-2} → artifact-2.md
   │              ↓
   │        returned evidence
   ├──── compare evidence and send follow-up if needed
   └──── Read artifacts → parent integration → 최종 산출물
```
...
| 상황 | 전략 |
|------|------|
| 팀원 1명 실패/중지 | parent가 감지 → 입력과 실패 사유를 좁혀 1회 재시도 → 재실패 시 누락 명시 |
| 팀원 과반 실패 | 사용자에게 알리고 진행 여부 확인 |
| 타임아웃 | 현재까지 수집된 부분 결과 사용, 미완료 팀원 종료 |
| 팀원 간 데이터 충돌 | 출처 명시 후 병기, 삭제하지 않음 |
| 작업 상태 지연 | parent task plan에서 blocked로 표시하고 재할당 또는 부분 진행 |
...
4. Phase 3에서 parent-mediated coordination으로 작업 수행
...
2. 리더가 유휴 알림 수신
3. parent가 실패 사유와 입력 범위를 좁혀 1회 재시도
...
팀 조율 오버헤드가 불필요한 경우. parent가 단일 custom subagent를 dispatch하거나 직접 실행하고 반환값으로 결과를 수집한다.
...
### Phase 2: 병렬 실행
독립성이 확인된 작업만 bounded parallel로 dispatch:

| 에이전트 | 입력 | 출력 | dispatch 방식 |
|---------|------|------|----------------|
| {agent-1} | {소스} | `_workspace/{phase}_{agent}_{artifact}.md` | bounded parallel 가능 |
| {agent-2} | {소스} | `_workspace/{phase}_{agent}_{artifact}.md` | bounded parallel 가능 |
...
N개 에이전트를 독립 작업으로 bounded parallel dispatch.
각 결과는 `_workspace/02_{agent}_raw.md`에 저장.

### Phase 3: 합의 기반 통합
**실행 모드:** 에이전트 팀

1. parent가 통합 subagent 묶음 구성 (editor + fact-checker + synthesizer)
2. parent task plan으로 작업 분배 — 모두 Phase 2의 `_workspace/02_*` 파일을 Read
3. parent가 상충 데이터를 각 subagent follow-up 입력으로 전달하고, 파일 기반으로 합의안 도출
4. parent가 최종 통합본 `_workspace/03_integrated.md` 생성
5. Phase 3 subagent 결과를 보존하고 다음 Phase로 진행
...
**하이브리드 전환 규칙:**
- 팀 → 서브: 현재 Phase의 subagent 결과와 `_workspace/` 산출물을 보존한 후 다음 subagent를 dispatch
- 서브 → 팀: 서브 에이전트의 파일 산출물을 팀원들에게 Read 경로로 전달
- 팀 → 팀: 이전 팀의 결과를 parent가 비교·정리한 후 새 subagent 묶음을 dispatch
...
2. **팀 모드는 parent dispatch, follow-up, task plan 사용법을 구체적으로** — 팀 구성, 작업 등록, 통신 규칙
3. **서브 모드는 agent file, prompt, 입력/출력, bounded parallel 여부를 완전히 명시** — 모델은 지정하지 않음
...
준비 → Phase 0(컨텍스트 확인) → parent dispatch plan + task plan → N개 팀원 병렬 실행 → Read + 통합 → 정리.
