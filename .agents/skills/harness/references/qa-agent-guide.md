...
### 3-1. QA 에이전트 권한을 명시하라

QA 에이전트는 기본적으로 `sandbox_mode = "read-only"`로 정의한다. 효과적인 QA는:
- Grep으로 패턴 검색 (모든 `NextResponse.json()` 추출)
- 스크립트 실행으로 자동 대조 (API shape vs 훅 타입)
- 필요 시 수정 요청까지 가능

**권장**: `.codex/agents/qa-inspector.toml`에 읽기 중심 QA 역할을 정의하고, 에이전트 정의에서 "검증 → 리포트 → 수정 요청" 프로토콜을 명시한다. 실제 수정은 parent가 최종 판단 후 수행한다.
...
- 발견 즉시 parent에게 구체적 수정 요청을 반환한다 (파일:라인 + 수정 방법)
- 경계면 이슈는 생산자와 소비자 양쪽에 전달해야 할 follow-up 질문을 함께 반환한다
- parent에게 검증 리포트(통과/실패/미검증 항목 구분)를 반환한다
- 직접 파일을 수정하지 않고, evidence와 권장 수정안을 parent에게 넘긴다
"""
```
