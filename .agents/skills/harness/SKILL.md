...
1. `프로젝트/.codex/agents/`, `프로젝트/.agents/skills/`, `프로젝트/AGENTS.md`를 읽는다
...
**에이전트 팀이 최우선 기본값이다.** 2개 이상의 에이전트가 협업할 때는 반드시 parent-orchestrated subagent team을 먼저 검토한다. Codex에서는 팀원끼리 직접 대화하지 않으므로 parent가 각 subagent의 역할·입력·의존성을 명시하고, 결과를 수집한 뒤 follow-up을 다시 보내 교차 검증·상충 토론·누락 보완을 재현한다.

| 모드 | 언제 사용 | 특성 |
|------|----------|------|
| **에이전트 팀** (기본) | 2명 이상 협업, parent가 중간 산출물을 상호 참조시켜야 함 | parent가 subagent들을 명시적으로 dispatch하고 task plan으로 의존성을 추적하며, 반환 결과 기반 follow-up으로 조율 |
| **서브 에이전트** (대안) | 단일 에이전트 작업, 결과만 parent에 반환하면 충분, 팀 조율 오버헤드가 과할 때 | 단일 Codex custom subagent dispatch 또는 parent 직접 실행 |
...
신규 에이전트 생성 전, `프로젝트/.codex/agents/`의 기존 에이전트와 중복 여부를 확인한다. 하네스를 반복 구축하다 보면 역할이 겹치는 에이전트가 다른 이름으로 누적되기 쉽다.

> 중복 분류 기준과 재사용 설계는 `references/agent-design-patterns.md`의 "에이전트 재사용 설계" 참조.

**모든 에이전트는 반드시 `프로젝트/.codex/agents/{name}.toml` 파일로 정의한다.** 에이전트 정의 파일 없이 일회성 prompt에 역할을 직접 넣는 것은 금지한다. 이유:
- 에이전트 정의가 파일로 존재해야 다음 세션에서 재사용 가능
- parent-mediated coordination protocol이 명시되어야 에이전트 간 협업 품질 보장
- 하네스의 핵심 가치는 에이전트(누가)와 스킬(어떻게)의 분리

분석·검토·QA 에이전트는 기본적으로 `sandbox_mode = "read-only"`로 둔다. 실제 파일 생성이나 수정이 필요한 production agent만 명시적으로 더 넓은 권한을 선택한다.

**모델 설정:** custom agent TOML에는 모델을 지정하지 않는다. Codex 런타임의 현재 모델 선택을 그대로 사용한다.

**팀 재구성:** Phase별로 다른 전문가 조합이 필요하면, 이전 subagent 결과를 `_workspace/`에 보존한 뒤 parent가 다음 Phase의 subagent 묶음을 새로 dispatch한다.

각 에이전트를 `프로젝트/.codex/agents/{name}.toml`에 정의한다. 필수 필드: `name`, `description`, `sandbox_mode`, `developer_instructions`. `developer_instructions`에는 핵심 역할, 작업 원칙, 입력/출력 프로토콜, 에러 핸들링, parent coordination protocol을 포함한다.

> 정의 템플릿과 실제 파일 전문은 `references/agent-design-patterns.md`의 "에이전트 정의 구조" + `references/team-examples.md` 참조.

**QA 에이전트 포함 시 필수 사항:**
- QA 에이전트는 기본적으로 `sandbox_mode = "read-only"`로 두되, 검증 스크립트 실행이 필요한 프로젝트에서는 parent가 명시적으로 실행 가능한 환경을 제공한다
...
신규 스킬 생성 전, `프로젝트/.agents/skills/`의 기존 스킬과 중복 여부를 확인한다. 하네스를 반복 구축하다 보면 기능이 겹치는 스킬이 다른 이름으로 누적되기 쉽다.
...
**에이전트 팀 패턴 (기본):**
오케스트레이터가 parent 역할을 맡아 custom subagent들을 명시적으로 dispatch하고, task plan으로 의존성을 추적한다. 팀원 간 직접 통신 대신 parent가 각 결과를 비교해 필요한 follow-up을 보내며, 상충 evidence는 삭제하지 않고 출처와 함께 병기한다.

```
[오케스트레이터/리더]
    ├── dispatch subagents with explicit roles and inputs
    ├── maintain task plan with dependencies
    ├── collect returns and workspace artifacts
    ├── send parent-mediated follow-up when evidence conflicts
    ├── 결과 수집 및 종합
    └── close subagent threads and preserve workspace
```

**서브 에이전트 패턴 (대안):**
오케스트레이터가 단일 custom subagent 또는 parent 직접 실행으로 작업을 처리한다. 병렬 실행이 필요하면 독립 작업만 bounded parallel로 dispatch하고, 결과는 parent가 모두 기다린 뒤 수집한다. 팀 조율이 불필요하고 오버헤드를 줄이고 싶을 때 사용.

```
[오케스트레이터]
    ├── dispatch agent-1 only if needed
    ├── dispatch agent-2 only if independent
    ├── 결과 대기 및 수집
    └── 통합 산출물 생성
```
...
- **Phase 간 팀 재구성**: 각 Phase마다 기존 subagent thread를 종료하고 `_workspace/`를 보존한 뒤, 다음 Phase의 subagent 묶음을 새로 dispatch
...
| 전략 | 방식 | 적용 모드 | 적합한 경우 |
|------|------|----------|-----------|
| **메시지 기반** | parent가 한 subagent의 반환 결과를 다른 subagent에게 follow-up 입력으로 전달 | 팀 | 피드백 교환, 상충 evidence 확인, 가벼운 상태 전달 |
| **태스크 기반** | parent task plan으로 작업 상태와 의존성을 추적 | 팀 | 진행상황 추적, 의존 관계 관리, 부분 재실행 범위 산정 |
| **파일 기반** | 약속된 경로에 파일을 쓰고 읽음 | 팀 + 서브 | 대용량 데이터, 구조화된 산출물, 감사 추적 필요 |
| **반환값 기반** | custom subagent의 반환 메시지 | 서브 | subagent 결과를 parent가 직접 수집 |
...
**AGENTS.md에 넣지 않는 것:** 에이전트 목록, 스킬 목록, 디렉토리 구조, 실행 규칙 상세. 이유: 에이전트/스킬 목록은 오케스트레이터 스킬과 `.codex/agents/`, `.agents/skills/`에서 관리하므로 중복이다. 디렉토리 구조는 파일 시스템에서 직접 확인 가능하다. AGENTS.md는 **포인터(트리거 규칙) + 변경 이력**만 담는다.
...
각 에이전트 `.toml` 파일에 "이전 산출물이 있을 때의 행동"을 명시한다:
...
- **서브 에이전트**: 각 에이전트의 입출력 연결, bounded parallel 대상의 독립성, 반환값 수집 로직 확인
...
|------|----------|------|------|
| 2026-04-05 | 초기 구성 | 전체 | - |
| 2026-04-07 | QA 에이전트 추가 | .codex/agents/qa.toml | 산출물 품질 검증 부족 피드백 |
...
- [ ] `프로젝트/.codex/agents/` — **에이전트 정의 파일 필수 생성** (빌트인 타입이라도 파일 생성 필수)
